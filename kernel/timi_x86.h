 ...
 * the timi-jit-test harness in the Phase 1/3 host toolchain) and under this
 * freestanding x86_64 kernel build. kernel/timi_translate.c wraps this with
 * kernel-specific buffer/mapping calls. See AeroSLS-TIMI-ISA-v0.1.md §7
 * Phase 3 and §9 for the design.
 *
 * This file is a direct, unmodified copy of the host toolchain's
 * timi_x86.h — see that copy for the full "Phase 3 v1 scope, stated up
 ...
 * same bugs or lack thereof. See AeroSLS-TIMI-ISA-v0.1.md §13 for how this
 * kernel binds rt_resolve_fn/rt_objsize_fn/rt_objtype_fn to real
 * object_catalog[] queries (kernel/timi_runtime.c) where the host
