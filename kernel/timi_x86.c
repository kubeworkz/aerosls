/*
 * timi_x86.c — Phase 3 TIMI-to-x86-64 native translator (kernel copy).
 * Byte-identical to the host toolchain's timi_x86.c, verified there by
 * timi-jit-test: real execution of this exact encoder's output on x86-64
 ...
 * LEA/PTRADD, LOADI/LOADI64) — see AeroSLS-TIMI-ISA-v0.1.md §9. No libc
 ...
 * constants) — verified on the host side by timi-jit-test executing this
 * exact encoder's output against a mock object catalog; see
 * AeroSLS-TIMI-ISA-v0.1.md §13. This kernel binds those three function
 * pointers to kernel/timi_runtime.c's real object_catalog[] queries
 * instead (see kernel/timi_translate.c).
 *
 ...
 * verified on the host side by timi-jit-test actually executing a forged-
 * capability attempt on real hardware and confirming it's rejected. See
 * AeroSLS-TIMI-ISA-v0.1.md §14.
 */
#include "timi_x86.h"
