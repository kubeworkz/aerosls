/*
 * simi_riscv.c — SIMI-to-RV64 native translator (kernel copy).
 *
 * This file is a direct, unmodified copy of the host toolchain's
 * simi_riscv.c (tools/simi/simi_riscv.c) below this comment — see that
 * copy for the full "RV64 design decisions, stated up front" block
 * (naive load-operate-store codegen mirroring simi_x86.c, the t0/t1/t2
 * working-register convention, real jal/jalr CALL/RET, the auipc+ld
 * literal-pool technique standing in for x86's movabs, RV64 M-extension
 * MUL/DIV/MOD, the emit_cmp() relation synthesis, and the v0.3 capability-
 * tag region). No libc dependency, same freestanding discipline as
 * kernel/simi_x86.c.
 *
 * Gap Remediation SIMI Phase 9: this is the "groundwork" half of RISC-V
 * kernel wiring (see kernel/simi_riscv.h's top comment for the full scope
 * statement and AeroSLS-SIMI-ISA-v0.1.md §16 Phase 9 for the honest
 * findings). The translator itself is verified correct by the host
 * toolchain's rv64_exec.c-based three-way regression suite (§12); nothing
 * about compiling this same source under this kernel's freestanding build
 * changes that correctness argument — it's the same encoder, unmodified.
 * What's NOT here yet: a kernel/simi_translate_riscv.c-equivalent (the
 * kernel/simi_translate.c analog) wiring this into a live activation/
 * spawn path — that needs real RISC-V user-mode paging, an activation
 * cache, and an object-catalog/syscall-dispatch/exit-stub story that
 * doesn't exist yet, exactly the blocker the roadmap doc named up front.
 */
