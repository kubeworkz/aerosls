...
// Creates a new group with the given name and role. Returns 1 on success,
// 0 if the name is already taken or the table is full (bump-allocated, no
// reclaim -- same fixed-size-registry posture as role_table[]/auth_tokens[]
// elsewhere in this codebase; a destroy/reclaim path is a real future
// improvement, not attempted in this first pass).
int group_create(const char* name, SLSRole role);

// Adds uid to the named group's member list. Returns 1 on success, 0 if the
// group doesn't exist, uid is already a member, or the member list is full.
int group_add_member(const char* name, uint32_t uid);

// Returns 1 if uid is an active member of the named (active) group, 0
// otherwise (including if the group doesn't exist).
