/*
 * timi_translate.c — Phase 3 kernel-side glue: runs timi_x86_translate()
 * (kernel/timi_x86.c, byte-identical to the host toolchain's copy — see
 * AeroSLS-TIMI-ISA-v0.1.md §9) against an uploaded TIMI ServiceBinary and
 ...
 * TIMI object to native code happens at most once per distinct object
 ...
 * of re-running timi_x86_translate().
 *
 ...
 * trampoline code (see timi_x86.c / AeroSLS-TIMI-ISA-v0.1.md §10) is
 * correct for every process without re-translating. The one thing that
 * must NOT be shared is the scratch page's physical backing — TIMI code
 ...
 * content is detected (hash/size mismatch) and forces a retranslation. The
 * old cached frames are simply overwritten in the activation slot and not
 * reclaimed — this kernel has no frame-free path anywhere yet (see
 * loader.c's ServiceBinary store, kernel/frame_pool.c), so a changed TIMI
 * object leaks its old native-code frames exactly the way every other
 * static-array resource in this kernel does today. Flagged, not solved —
 * same spirit as the OBJ_TYPE_PROGRAM staging gap (§9) and the DIV/MOD #DE
 * gap (§10).
 *
 ...
 * call sites for no test-suite benefit yet — every TIMI object in this
...
#include "timi_x86.h"
#include "timi_runtime.h"      /* Phase 6 (v0.3): real RESOLVE/OBJSIZE/OBJTYPE bindings */
#include "loader.h"
#include "kernel_io.h"
 ...
 * LOADER_MAX_BINARY_SIZE (16 KiB of TIMI bytecode): the naive
...
#define TIMI_CODE_BUF_SIZE (192 * 1024)
static uint8_t g_timi_code_buf[TIMI_CODE_BUF_SIZE];

/* ── Phase 4: activation cache ──────────────────────────────────────────── */
/* One slot per distinct TIMI object name — mirrors MAX_SERVICE_BINARIES
 * since there is at most one live activation per uploaded TIMI object.
...
#define TIMI_MAX_ACTIVATIONS   MAX_SERVICE_BINARIES
#define TIMI_ACT_MAX_PAGES     (TIMI_CODE_BUF_SIZE / 4096)   /* 48 pages */

struct TimiActivation {
    ...
    uint32_t code_pages;
    uint32_t entry_off;
    uint64_t frame[TIMI_ACT_MAX_PAGES];    /* physical addrs; page i maps at base_vaddr+i*4096 */
};

static struct TimiActivation g_activations[TIMI_MAX_ACTIVATIONS];
...
static struct TimiActivation* tt_find_activation(const char* object_name) {
    for (int i = 0; i < TIMI_MAX_ACTIVATIONS; i++) {
...
static struct TimiActivation* tt_find_or_alloc_activation(const char* object_name) {
...
uint64_t timi_translate_and_map(const char* object_name, uint64_t base_vaddr, uint64_t* pml4) {
    ...
    if (!sb || !sb->is_timi) {
        kernel_serial_printf(
            "[TIMI] translate: '%s' is not a loaded TIMI object.\n", object_name);
        return 0;
    }

    uint32_t hash = tt_fnv1a(sb->data, sb->size);
    struct TimiActivation* act = tt_find_or_alloc_activation(object_name);
    ...
    if (act && act->valid && act->content_hash == hash && act->content_size == sb->size) {
        ...
        if (!scratch_frame) {
            kernel_serial_print("[TIMI] translate: out of memory mapping scratch page.\n");
            return 0;
        }
        ...
        kernel_serial_printf(
            "[TIMI] '%s' activation cache HIT — reused %u page(s), skipped "
     ...
     * kernel functions (timi_runtime.c, backed by the live
     ...
     * timi_x86.h's own (uint64_t)(uintptr_t) casts for the same reason). */
    uint64_t rt_resolve = (uint64_t)(uintptr_t)timi_rt_resolve;
    uint64_t rt_objsize = (uint64_t)(uintptr_t)timi_rt_objsize;
    uint64_t rt_objtype = (uint64_t)(uintptr_t)timi_rt_objtype;

    uint32_t len1 = 0, off1 = 0;
    int rc = timi_x86_translate(sb->data, sb->size, g_timi_code_buf, TIMI_CODE_BUF_SIZE,
                                 "main", 0, rt_resolve, rt_objsize, rt_objtype, &len1, &off1);
    if (rc != TX_OK) {
        kernel_serial_printf("[TIMI] translate: '%s' failed: %s\n",
                             object_name, timi_x86_strerror(rc));
        return 0;
    }
    ...
    rc = timi_x86_translate(sb->data, sb->size, g_timi_code_buf, TIMI_CODE_BUF_SIZE,
                             "main", scratch_vaddr, rt_resolve, rt_objsize, rt_objtype, &len2, &off2);
    if (rc != TX_OK || len2 != len1) {
        kernel_serial_printf(
            "[TIMI] translate: '%s' second pass mismatch — internal translator bug, "
            "refusing to map possibly-inconsistent code.\n", object_name);
        return 0;
    }

    /* Append a small kernel-specific outer stub, deliberately kept out of
     * the portable timi_x86.c (that file always ends its trampoline in a
     * plain `ret` so it stays callable/testable as a normal function on
     * the host — see timi_x86.h). Here there's no caller to return to, so
     * this stub calls the translated trampoline for a real return address,
     * then turns RAX (the TIMI program's r0) into a SYS_SLS_EXIT syscall:
     ...
     */
    uint32_t p = len2;
    if (p + 32 > TIMI_CODE_BUF_SIZE) {
        kernel_serial_print("[TIMI] translate: no room for exit stub.\n");
        return 0;
    }
    uint8_t* buf = g_timi_code_buf;
    ...
    if (total_pages > TIMI_ACT_MAX_PAGES) {
        /* Can't happen given TIMI_CODE_BUF_SIZE == TIMI_ACT_MAX_PAGES*4096
         * and the p+32>TIMI_CODE_BUF_SIZE check above, but keep the
         * invariant explicit rather than silently overrunning act->frame[]. */
        kernel_serial_print("[TIMI] translate: translated code exceeds activation cache capacity.\n");
        return 0;
    }
    ...
    for (uint32_t i = 0; i < total_pages; i++) {
        ...
        if (!frame) {
            kernel_serial_print("[TIMI] translate: out of memory mapping native code.\n");
            return 0;
        }
        ...
        tt_memcpy(frame, g_timi_code_buf + off, chunk);
    ...
    if (act) {
        ...
        kernel_serial_printf(
            "[TIMI] '%s' translated but the activation table is full (%d slots) — "
            "this translation will not be cached; every spawn will retranslate.\n",
            object_name, TIMI_MAX_ACTIVATIONS);
    }

    kernel_serial_printf(
        "[TIMI] '%s' activation cache MISS — translated %u bytes native code "
