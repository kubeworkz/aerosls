/*
 * simi_runtime.c — Phase 6 (v0.3) real object-catalog runtime for SIMI's
 * RESOLVE/OBJSIZE/OBJTYPE opcodes. See simi_runtime.h for the contract and
 * AeroSLS-SIMI-ISA-v0.1.md §13 for the design. No libc dependency, same
 * discipline as every other kernel source file that touches translated
 * SIMI code paths.
 *
 * Phase 7 adds an authority check to simi_rt_resolve(): before handing
 * back a base_vaddr (which the x86/RV64 translators will then tag as a
 * capability — see simi_x86.h's Phase 7 notes), it looks up the calling
 ...
 * See AeroSLS-SIMI-ISA-v0.1.md §14.
 */
#include "simi_runtime.h"
#include "object_catalog.h"
#include "process.h"
#include "../user/permissions.h"

#define SIMI_RT_NAME_MAXLEN 32   /* must match simi_isa.h's SIMI_MAX_NAME */
#define SIMI_RT_PAGE_SIZE   4096ULL

/* Bounded name compare: `name_ptr` is a raw pointer into the calling SIMI
 * object's own name-pool bytes (see simi_x86.h's TX_NAMEPOOL_ARG_IDX) —
 * guaranteed NUL-terminated within SIMI_RT_NAME_MAXLEN bytes by the
 * assembler that produced the object (tools/simi/simi_asm.c's
...
static int rt_name_matches(const char* name_ptr, const char* catalog_name) {
    for (int i = 0; i < SIMI_RT_NAME_MAXLEN; i++) {
        char a = name_ptr[i];
...
uint64_t simi_rt_objsize(uint64_t base_vaddr) {
    ...
    for (uint32_t i = 0; i < object_catalog_count; i++) {
        ...
        if (object_catalog[i].base_vaddr == base_vaddr) {
            return (uint64_t)object_catalog[i].size_pages * SIMI_RT_PAGE_SIZE;
        }
    }
    return 0;
}
