#!/usr/bin/env bash
# tests/riscv_kernel_copy_rediff_smoke.sh — teeth for the RV64 kernel-copy
# re-diff tripwire (tests/riscv_kernel_copy_rediff_check.sh).
#
# ─── Why this exists ───────────────────────────────────────────────────────
# The check proves the kernel/simi_riscv.{c,h} bodies are byte-identical
# to the tools copies; this proves the CHECK has teeth. It copies the
# real files to a temp dir, mutates one byte-stream in a throwaway copy,
# points the check at the copies via its positional args, and asserts the
# check FAILS each time — plus asserts the pristine copies still PASS (so
# the teeth are meaningful, not trivially red). A check that has gone
# blind (e.g. its marker extraction broke and it now diffs empty bodies)
# fails here. The real files are never touched. Mirror of
...
CHECK="tests/riscv_kernel_copy_rediff_check.sh"
TMPD=$(mktemp -d)
trap 'rm -rf "$TMPD"' EXIT

cp tools/simi/simi_riscv.c "$TMPD/host.c"
cp kernel/simi_riscv.c     "$TMPD/kernel.c"
cp tools/simi/simi_riscv.h "$TMPD/host.h"
cp kernel/simi_riscv.h     "$TMPD/kernel.h"

pass=0
fail=0

echo "riscv64 kernel-copy re-diff smoke"
echo "================================="
echo
...
sed 's/emit_li64/emit_li64_X/' "$TMPD/kernel.c" > "$TMPD/kernel_mut.c"
...
sed 's/emit_li64/emit_li64_Y/' "$TMPD/host.c" > "$TMPD/host_mut.c"
