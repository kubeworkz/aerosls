#!/usr/bin/env bash
# tests/arm_kernel_copy_rediff_smoke.sh — teeth for the kernel-copy
# re-diff tripwire (tests/arm_kernel_copy_rediff_check.sh).
#
# ─── Why this exists ───────────────────────────────────────────────────────
# The check proves the kernel/simi_arm.{c,h} bodies are byte-identical to
...
# here. The real files are never touched.
#
...
CHECK="tests/arm_kernel_copy_rediff_check.sh"
TMPD=$(mktemp -d)
trap 'rm -rf "$TMPD"' EXIT

cp tools/simi/simi_arm.c "$TMPD/host.c"
cp kernel/simi_arm.c     "$TMPD/kernel.c"
cp tools/simi/simi_arm.h "$TMPD/host.h"
cp kernel/simi_arm.h     "$TMPD/kernel.h"

pass=0
fail=0

echo "arm kernel-copy re-diff smoke"
echo "============================="
echo
...
sed 's/TX_AR_OK/TX_AR_OK_X/' "$TMPD/kernel.c" > "$TMPD/kernel_mut.c"
...
sed 's/TX_AR_OK/TX_AR_OK_Y/' "$TMPD/host.c" > "$TMPD/host_mut.c"
