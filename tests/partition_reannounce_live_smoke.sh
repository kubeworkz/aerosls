...
# one: the guard's pid file, node logs and relaunch stderr live in a private
# temp dir (AEROSLS_CLUSTER_DIR), so it runs beside a live cluster, and it
# asserts at the end that the repo's cluster/cluster.pids is unchanged.
#
...
# The guard kills and relaunches whatever cluster.pids names (and writes the
# relaunched follower's stderr beside it), and the deploy gate runs this
# smoke on the host where the real cluster is live -- with a shared cluster/
# the only safe move was to refuse, and a smoke that cannot run proves
# nothing (the 2026-09-13 deploy gate failed on exactly that).
# AEROSLS_CLUSTER_DIR is exported so EVERY guard call below, the silent
# no-cluster tooth included, resolves its files here and can never read a
# real cluster's.
