...
# spawning a SIMI program against a real, booted AeroSLS kernel, and
# confirming the translated native code computed the right answer.
#
# Everything up to this point (Phase 2's simi_validate() round-trip test,
...
#
# What it proves, concretely: this uploads tests/add.tmo (SIMI bytecode:
...
#
#   [SIMI] 'simi_add_test' activation cache MISS — translated ... bytes ...
#   [PROC] PID <n> 'simi_add_test' exited (code=5).
#
...
# add.simi's SIMI bytecode computes. Not a simulation, not a host-side
...
PROGRAM_NAME="${PROGRAM_NAME:-simi_add_test}"
TMO_FILE="${TMO_FILE:-$(dirname "$0")/tests/add.tmo}"

if [ ! -f "$TMO_FILE" ]; then
    echo "Building $TMO_FILE (via simi-asm)..."
    make -C "$(dirname "$0")" simi-asm >/dev/null
    "$(dirname "$0")/simi-asm" "$(dirname "$0")/tests/add.simi" "$TMO_FILE"
fi
...
echo ""
echo "      [SIMI] '$PROGRAM_NAME' activation cache MISS — translated ..."
echo "      [PROC] PID <n> '$PROGRAM_NAME' exited (code=5)."
echo ""
echo "    code=5 is the real proof — that's add.simi's expected result,"
