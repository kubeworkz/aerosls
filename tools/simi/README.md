...
Host-side (Phase 1, 3, 5) tooling for AeroSLS's SIMI bytecode — the
translated-not-interpreted virtual ISA that SLIC compiles into native code,
modeled on IBM i's SIMI/SLIC/LPAR architecture. Full design rationale is in
`../../docs/AeroSLS-SIMI-ISA-v0.1.md`.
...
`.simi` programs against two different native targets without booting the
kernel.
...
```
cd tools/simi
make               # builds simi-asm, simi-run, simi-dis, simi-jit-test, simi-riscv-verify
make test          # assembles + interprets every tests/*.simi, checks expected results
...
- `simi-asm <file>.simi` — two-pass assembler, emits a `.tmo` object file.
- `simi-run <file>.tmo` — reference interpreter (Phase 1). Simplified but
  unambiguous semantics: no width truncation, floats unimplemented, LOAD/STORE
  against a simulated 64KB memory array.
- `simi-dis <file>.tmo` — disassembler, resolves branch/call targets back to
  labels.
- `simi-jit-test <file>.tmo <entry> <expected>` — Phase 3 harness: translates
  the object to real x86-64 machine code via `simi_x86.c`, mmaps it
...
- `simi-riscv-verify <file>.tmo <entry> <expected>` — Phase 5 harness:
  translates the object to RV64 machine code via `simi_riscv.c`, then
  ...
  `simi_riscv_verify.c`'s top comment and §12 of the design doc for what
  that does and doesn't prove.

## Two native targets, one bytecode format

`simi-asm` produces the same `.tmo` regardless of which target you're
translating for — `simi_x86.c` and `simi_riscv.c` both consume the exact
same object format (`simi_isa.h`'s on-disk layout). Running the same
...
v0.2, see `simi_isa.h`'s top comment.
...
- Phase 1 interpreter (`simi_interp.c`), the x86 JIT harness
  (`simi_jit_test.c`), and the RV64 verify harness (`simi_riscv_verify.c`)
  all bind them to small, identical mock catalogs — that's why
  `tests/obj_ops.simi` produces the same result (305) everywhere.
- `kernel/simi_runtime.c` binds them to the real, live `object_catalog[]`.
...
- **Tagged, unforgeable capabilities.** Every symbolic register gets a
  ...
  isn't. `tests/cap_forge.simi` proves the forgery path is actually closed:
...
- **Authority-checked minting.** `kernel/simi_runtime.c`'s `simi_rt_resolve()`
...
`simi_x86.c` / `simi_x86.h` here are the copy of record. `kernel/simi_x86.c`
and `kernel/simi_x86.h` are meant to be **byte-identical** (modulo the doc
comments) to these — that's deliberate: it's what makes `simi-jit-test`'s
...
diff to confirm they still match. `kernel/simi_runtime.c` has no host
...
catalogs live directly in `simi_interp.c`/`simi_jit_test.c`/
`simi_riscv_verify.c` instead).

`simi_riscv.c` / `simi_riscv.h` have **no kernel copy yet** — the RISC-V side
...
infrastructure existing first — a separate, non-SIMI undertaking. See §12.
