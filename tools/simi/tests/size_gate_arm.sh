...
# lcg_slice is the arm64 kernel-embedded LCG fixture (measured at this
# commit — post-M0, same discipline as lcg_fairness): one full per-slice
# LCG budget (200,000 iterations) returning the raw acc tooth
# 0x0b6f2a40, whose real gate is the arm64 kernel boot's translate +
# execute + serial assert (plan doc §10.200), NOT the parity corpus
# (it is skipped there and in the bench tools — the arm64_boot_smoke
# model). The ARM runner still executes it (its ~4.4M A64 steps fit the
...
# M0-era discipline applies from here on): the kernel's LCG work loop
# translated into SIMI, one full per-slice budget (200,000 iterations ->
# acc = 0x0b6f2a40) plus the light baseline (100,000 -> 0x0abe2d20),
# self-checking both teeth (result 1 only if both match). The loop is
# ~6.7M A64 executed instructions (the parity harnesses' tight 10M step
# budget, which the kernel's 1M-iteration per-task total would exceed —
# see the fixture header). A codegen regression that bloats the loop
# body fails this row; the committed per-leg step baselines
# (bench_baselines*.h) pin the execution work on all four engines.
