/*
 * timi_disasm.c — Phase 1 TIMI disassembler.
 *
...
static const char *entry_name_at(TimiObject *obj, uint32_t idx) {
...
int main(int argc, char **argv) {
    ...
    for (uint32_t pc = 0; pc < obj.num_instr; pc++) {
        ...
        const char *mnem = (op < OP_COUNT) ? TIMI_OPS[op].name : "???";
        const char *tname = (type < T_COUNT) ? TIMI_TYPE_NAMES[type] : "?";
        TimiFmt fmt = (op < OP_COUNT) ? TIMI_OPS[op].fmt : FMT_NONE;

        printf("    @%-5u %-8s", pc, mnem);
        switch (fmt) {
        case FMT_RRR:
            if (flags & FLAG_IMM)
                printf("r%u, r%u, #%d, %s", rd, ra, timi_imm28(w), tname);
            else
                printf("r%u, r%u, r%u, %s", rd, ra, timi_rb_reg(w), tname);
            break;
        case FMT_RR:
        ...
        case FMT_MOV:
            ...
            break;
        case FMT_LOADI:
            printf("r%u, #%d, %s", rd, timi_imm28(w), tname);
            break;
        case FMT_LOADI64:
            printf("r%u, [pool %u], %s", rd, timi_rb_raw(w), tname);
            break;
        case FMT_CMP:
            printf("r%u, r%u, r%u, %s, %s", rd, ra, timi_rb_reg(w), tname,
                   flags < REL_COUNT ? TIMI_REL_NAMES[flags] : "?");
            break;
        case FMT_BR: {
        ...
        case FMT_STORE:
            ...
            break;
        case FMT_LEA:
            printf("r%u, r%u, #%d", rd, ra, timi_imm28(w));
            break;
        case FMT_PTRADD:
            if (flags & FLAG_IMM)
                printf("r%u, r%u, #%d, %s", rd, ra, timi_imm28(w), tname);
