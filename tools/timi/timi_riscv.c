/*
 * timi_riscv.c — Phase 5 TIMI-to-RV64 native translator. See timi_riscv.h
 * for the framing and AeroSLS-TIMI-ISA-v0.1.md §12 for design notes. No
 * libc dependency, mirrors timi_x86.c's discipline so this compiles
 ...
 *   - Same naive load-operate-store codegen as x86 (timi_x86.c): every
 ...
 *     reloaded from its stack slot at the start of each TIMI instruction's
 *     codegen and the result stored back immediately, no value is ever
 *     assumed to survive across a TIMI instruction boundary.
 *   - CALL/RET use real `jal ra,target` / `jalr x0,0(ra)` so nested and
 *     recursive TIMI calls get a real hardware-adjacent call chain, same
 ...
 *     short of TIMI's 28-bit LOAD/STORE/LEA/PTRADD displacement field.
 ...
 *     how RV64 works natively, TIMI's design principle #2 (no implicit
 ...
 * same enums as timi_x86.c, kept as an independent copy per this file's
 * zero-shared-header policy (see timi_riscv.h's top comment). */
enum {
 ...
 * Direct RV64 analog of timi_x86.c's rbp-relative reg_disp/ld_rax/st_rax,
 ...
 * garbage, not by review; see AeroSLS-TIMI-ISA-v0.1.md §12.)
 ...
 * unforged T_OBJREF capability, 0 = plain value. See timi_x86.c's mirror
 ...
 * rationale as timi_x86.c's emit_jz_placeholder/patch_local_rel32: the
 * branch target is a few instructions further into this same emitted
 * TIMI instruction's code, not another TIMI pc, so it doesn't go through
 ...
 * directly for TIMI's full 28-bit displacement range. */
 ...
 * Same two-pass shape as timi_x86.c's g_fixups, adapted for RV64's
...
static uint32_t g_instr_off[4096];   /* TX_MAX_INSTR, mirrors timi_x86.c */

/* v0.3 (Phase 6): translate-time context for RESOLVE/OBJSIZE/OBJTYPE
 * codegen — same file-scope-global convention as timi_x86.c's g_rt_*. */
 ...
 * TIMI's BC tests a single register for zero/nonzero, see emit BC below. */
...
/* ─── LOAD/STORE width+signedness (mirrors type_width() in timi_x86.c and
 * the Phase 1 reference interpreter) — RV64 has a native load/store
 * opcode for every TIMI width/signedness combination, so unlike x86 there
...
static void emit_prologue(struct CodeBuf* cb) {
     ...
     * untagged, matching "capabilities don't cross CALL" (incoming args
     * are copied in via plain st_slot below, onto already-zeroed tag
     * bytes, same as x86's prologue). */
 ...
 * live there) into rd's slot. Mirrors timi_x86.c's emit_call_site. ─────*/
static void emit_call_site(struct CodeBuf* cb, uint32_t target_pc, int rd) {
    i_addi(cb, X_SP, X_SP, -64);
    int nargs = TX_RV_MAX_REGS < 8 ? TX_RV_MAX_REGS : 8;
    for (int i = 0; i < nargs; i++) {
        ld_slot(cb, X_T0, i);
    ...
    i_addi(cb, X_SP, X_SP, 64);
    /* v0.3 (Phase 7): capabilities don't cross a CALL boundary in this v1
     * — untag the destination regardless of what RET did in the callee,
     * mirrors x86's emit_call_site's st_rax_untag. */
...
static int emit_instr(struct CodeBuf* cb, uint64_t w) {
    ...
    case OP_PTRADD: {
         ...
         * unforgeability property (see timi_x86.c's identical note). */
    ...
    case OP_RESOLVE: {
        /* rb_raw holds the name-pool index (FMT_RESOLVE, timi_isa.h). a0 =
    ...
    case OP_OBJSIZE: case OP_OBJTYPE: {
         ...
         * is what closes the forgery hole (see timi_x86.c's identical
         * note and tests/cap_forge.timi). */
...
int timi_riscv_translate(const uint8_t* obj_data, uint32_t obj_size,
    ...
    if (!obj_data || obj_size < sizeof(struct TimiObjHdrRV)) return TX_RV_ERR_BAD_HEADER;
    struct TimiObjHdrRV hdr;
    rv_memcpy(&hdr, obj_data, sizeof(hdr));
    if (hdr.magic != TIMI_RV_MAGIC) return TX_RV_ERR_BAD_HEADER;

    uint64_t expect = sizeof(struct TimiObjHdrRV);
    ...
    const uint64_t* instrs = (const uint64_t*)(obj_data + sizeof(struct TimiObjHdrRV));
     ...
     * own name-pool bytes — mirrors timi_x86.c exactly. */
    ...
    /* Literal pool, placed right after all code. */
...
const char* timi_riscv_strerror(int code) {
    switch (code) {
        case TX_RV_OK: return "ok";
        case TX_RV_ERR_BAD_HEADER: return "bad or corrupt TIMI header";
