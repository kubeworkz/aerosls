/*
 * timi_asm.c — Phase 1 TIMI assembler.
 *
 * Text .timi source -> binary .tmo object (see timi_isa.h / timi_obj.h).
 *
...
static char name_pool[MAX_NAMES][TIMI_MAX_NAME];
...
static int find_opcode(const char *name) {
    for (int i = 0; i < OP_COUNT; i++)
        if (strcasecmp(name, TIMI_OPS[i].name) == 0) return i;
    return -1;
}

static int find_type(const char *name) {
    for (int i = 0; i < T_COUNT; i++)
        if (strcasecmp(name, TIMI_TYPE_NAMES[i]) == 0) return i;
    return -1;
}

static int find_rel(const char *name) {
    for (int i = 0; i < REL_COUNT; i++)
        if (strcasecmp(name, TIMI_REL_NAMES[i]) == 0) return i;
    return -1;
}
 ...
 * exact string match. Truncates to TIMI_MAX_NAME-1 chars, same as
 * TimiName's fixed slot size. */
static int find_or_add_name(const char *name) {
    char buf[TIMI_MAX_NAME];
    strncpy(buf, name, TIMI_MAX_NAME - 1);
    buf[TIMI_MAX_NAME - 1] = 0;
...
static void assemble_instr(ClassifiedLine *c, int this_idx) {
    ...
    emit(timi_encode((uint8_t)op, (uint8_t)type, (uint16_t)rd, (uint16_t)ra, rb, (uint8_t)flags));
}
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: %s input.timi output.tmo\n", argv[0]);
        return 1;
    }
    cur_file = argv[1];
    ...
    for (int i = 0; i < nentry_names; i++) {
        ...
        strncpy(ent[nent].name, entry_names[i], TIMI_MAX_NAME - 1);
        ent[nent].name[TIMI_MAX_NAME - 1] = 0;
    ...
    TimiName names[MAX_NAMES];
    for (int i = 0; i < nname_pool; i++) {
        strncpy(names[i].name, name_pool[i], TIMI_MAX_NAME - 1);
        names[i].name[TIMI_MAX_NAME - 1] = 0;
    }
    obj.num_names = nname_pool;
    obj.names = names;

    if (timi_obj_write(argv[2], &obj) != 0) return 1;
