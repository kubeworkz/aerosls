/* timi_obj.c — .tmo object file read/write (v0.3: adds the name pool) */
#include "timi_obj.h"
#include <stdio.h>
#include <stdlib.h>
...
int timi_obj_write(const char *path, const TimiObject *obj) {
    ...
    if (obj->num_entries)  fwrite(obj->entries, sizeof(TimiEntry), obj->num_entries, f);
    if (obj->num_names)    fwrite(obj->names, sizeof(TimiName), obj->num_names, f);
...
int timi_obj_read(const char *path, TimiObject *obj) {
    ...
    if (fread(&obj->magic, sizeof(uint32_t), 1, f) != 1 || obj->magic != TIMI_MAGIC) {
        fprintf(stderr, "%s: not a TIMI object (bad magic)\n", path);
        fclose(f);
        return -1;
    }
    ...
    obj->entries = obj->num_entries ? malloc(sizeof(TimiEntry) * obj->num_entries) : NULL;
    obj->names = obj->num_names ? malloc(sizeof(TimiName) * obj->num_names) : NULL;
    ...
    if (obj->num_entries && fread(obj->entries, sizeof(TimiEntry), obj->num_entries, f) != obj->num_entries) {
        fprintf(stderr, "%s: truncated entry table\n", path);
        fclose(f); return -1;
    }
    if (obj->num_names && fread(obj->names, sizeof(TimiName), obj->num_names, f) != obj->num_names) {
