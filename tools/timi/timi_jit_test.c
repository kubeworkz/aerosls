/*
 * timi_jit_test.c — Phase 3 verification harness.
 *
 * This is the strongest verification available without a bootable AeroSLS
 * image: it translates a real .tmo (produced by the Phase 1 timi-asm) with
 * the exact same timi_x86.c that will be ported into the kernel, mmaps the
 ...
 *
 * Usage: timi-jit-test program.tmo entry_name expected_value
 */
#define _GNU_SOURCE
#include "timi_x86.h"
#include <stdio.h>
 ...
 * address (see timi_x86.h/.c). Deliberately the same names/sizes/types as
 * the Phase 1 reference interpreter's g_mock_catalog in timi_interp.c, so
...
int main(int argc, char** argv) {
    ...
    /* Scratch buffer for r7 (see timi_x86.h — the trampoline convention
     * standing in for real TIMI object/pointer allocation). A page-aligned
    ...
    int rc = timi_x86_translate(obj, (uint32_t)sz, code, cap,
                                 entry_name, (uint64_t)(uintptr_t)scratch,
                                 (uint64_t)(uintptr_t)timi_rt_resolve,
                                 (uint64_t)(uintptr_t)timi_rt_objsize,
                                 (uint64_t)(uintptr_t)timi_rt_objtype,
                                 &out_len, &entry_off);
    if (rc != TX_OK) {
        fprintf(stderr, "FAIL  %-28s translate error: %s\n", path, timi_x86_strerror(rc));
        return 1;
    }
