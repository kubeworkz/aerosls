/*
 * timi_riscv.h — Phase 5 TIMI-to-RV64 native translator: SLIC's second real
 * target, proving the same unmodified .tmo bytecode object retargets to a
 * different ISA without touching a single byte of the TIMI object itself.
 * That's the concrete claim behind "re-translation-on-migration" in the
 * roadmap (AeroSLS-TIMI-ISA-v0.1.md §7 Phase 5) — this file and
 * timi_riscv.c are the proof, not just an assertion of it.
 *
 * Portable, no-libc core, structured to mirror timi_x86.c function-for-
 ...
 * translators are easy to audit against each other. See timi_riscv.c's
 * top comment and AeroSLS-TIMI-ISA-v0.1.md §12 for the RV64-specific
 * design decisions (calling convention, the auipc+ld literal-pool
 * technique standing in for x86's movabs, branch/call encoding).
 *
 * This header intentionally does NOT reuse timi_x86.h's struct/macro
 * names (TimiObjHdr, TxEntryRec, TIMI_X_MAGIC) even though the on-disk
 ...
 *
 * v0.3 (Phase 7) adds the same capability-tag mechanism as timi_x86.c —
 ...
 * public-API change here either, entirely internal to timi_riscv.c's
 * codegen. Verified via tests/cap_forge.timi executing identically on
 * this target through rv64_exec.c. See AeroSLS-TIMI-ISA-v0.1.md §14.
 */
#ifndef TIMI_RISCV_H
#define TIMI_RISCV_H
...
/* v0.3 (Phase 6): num_names + TxNameRecRV added, mirroring timi_x86.h's
 * TimiObjHdr/TxNameRec. Same breaking change as the x86 side — see
 * timi_isa.h's top comment. */
struct TimiObjHdrRV {
    uint32_t magic;
    uint32_t num_instr;
...
#define TIMI_RV_MAGIC        0x314D4954u   /* "TIM1" — same object format as x86's TIMI_X_MAGIC */
...
#define TX_RV_NAME_SIZE        32             /* must match struct TxNameRecRV / TIMI_MAX_NAME */
...
/* Same contract as timi_x86_translate() (timi_x86.h) — see that file for
 ...
 * calling the entry procedure, standing in for real TIMI pointer
