...
Note that `make x86-run` forwards host **3001**, which is node 1's REST port. The launcher refuses rather than colliding; set `AEROSLS_HTTP_BASE` to move the range. `make x86-run` takes its own host port from `X86_HTTP_PORT` (default 3001, the port the README's copy-paste QEMU command and the production backend both use) and refuses to start when something already listens there, rather than dying later on "Could not set up host forwarding rule".

**Test boots never take a port from the live band.** `tests/free_port.sh` allocates every boot check's QEMU hostfwd from `AEROSLS_FREE_PORT_RANGE` — default **32001-32020**, deliberately outside `AEROSLS_HTTP_BASE+1 .. +CLUSTER_NODE_MAX` and the production backend on 3001 — and **refuses** a band that overlaps them instead of clamping it.

**And an allocated port is reserved, not merely probed.** A probe is a snapshot: two guard runs that both scan the band from the bottom both see the same port free, both hand it to their QEMU, and the loser dies on "Could not set up host forwarding rule" — the 2026-09-13 collision, one process later. So the allocator takes the port's lock file (`AEROSLS_FREE_PORT_LOCK_DIR`) *before* probing it, skips a port another run has locked **without** probing it, and keeps the lock in a detached holder until the port is bound — QEMU has taken it, which is what the reservation was for — or until `AEROSLS_FREE_PORT_HOLD_S` (default 300 s) expires, so a guard killed between allocating and launching cannot hold a port out of the band for good. `AEROSLS_FREE_PORT_HOLD=0` gives probe-only allocation back, and it has to be asked for: a host without `flock` gets a refusal naming that escape hatch, not a silent downgrade to probing, because probing alone is the defect. A squatter that does not take the lock is still caught only by the probe and by QEMU's own bind — the reservation narrows the window to the microseconds between the lock and the bind, it does not remove it.

The reason is the difference between *answering* and *trusting*. A probe protects a port something answers on; it cannot protect a port something else trusts without probing, and three things trust 3001: `tests/webapp_served_check.sh` (default URL), the inline QEMU steps in `.github/workflows/ci.yml`, and `make x86-run`. None of them can tell a boot-check kernel from the deployed one — both answer `/api/health`. On 2026-09-24 a local boot check probed 3001, found nothing answering, booted its kernel there, and the CI guard running on the same host compared *that* kernel's assets against the committed bundle: a red job whose cause was in none of the kernel, the guard, or the commit. So a host that runs the CI runner, or the live cluster, and also runs boot checks locally gives them a band of their own (`AEROSLS_FREE_PORT_RANGE`, and `X86_HTTP_PORT` for `make x86-run`). `tests/guard_port_band_check.sh` enforces the rule on every push: default band outside the live band, override honoured, overlap and malformed bands refused, a taken port skipped, an exhausted band failed closed, and no literal host port in the QEMU machinery.
...
| `qemu invl` | Exercises the §3.2 invalidation policy end to end. The guest edits its own page-table page in place (that store must fault and drop the stale subtree), executes `INVLPG`, then reloads `CR3`. Four stages, four distinct magic values — each can only be produced if the invalidation preceding it actually took effect, so a partial implementation fails at a nameable stage rather than looking like a general hang. |
| `qemu selfmod` | Self-modifying guest code with paging off. The guest stores over its own instruction bytes; that store must fault, bump the translated page's generation counter, and force a re-translation that serves the patched bytes. Passing means the translation cache noticed the code changed — which is exactly what a stale-cache bug would not do while otherwise appearing to run correctly. |
