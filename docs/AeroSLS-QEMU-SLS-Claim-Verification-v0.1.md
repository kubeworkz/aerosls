...
| Leader failover: a dead leader's role is taken over | `net/consensus.c` election, driven live via `tools/aeroslsctl` | **Verified on 2026-08-15** — live 3-node run (512 MiB/node, TCG, no KVM): node 2 was LEADER (term 1), SIGKILLed; node 1 campaigned and was elected LEADER for term 2, node 3 followed at term 2, quorum 2/3 intact, new leader served requests. Partition adoption (`failover_recover_from`) is NOT live-wired — covered only by `tests/checkpoint_recovery_host_test.c` | `./run-cluster.sh --nodes 3`, `aeroslsctl cluster status`, `kill -9 <node-2-pid>`, re-check `cluster status` |
