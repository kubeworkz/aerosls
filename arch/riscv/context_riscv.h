 ...
 * audit found GPR-only and with ZERO callers. The two-task FP
 * round-robin (kernel/kernel_riscv.c) is its first real caller, and the
 * FP-aware wrapper (rv_task_switch_fp) is what finally makes the switch
 * save and restore the RvPerHartData fp_save owner rows.
 *
...
struct RvTask {
    ...
    uint64_t fp_owner;     /* this task's FP owner id (0/1) into the shared
                              RvPerHartData fp_save registry (Design B part 2) */
