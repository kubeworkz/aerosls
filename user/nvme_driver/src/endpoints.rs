//! The set of RD_* endpoints the driver serves (the same design the ramdisk
//! driver carries, verbatim: the protocol surface is shared, so the endpoint
//! bookkeeping is shared).
//!
//! Adoption is idempotent and comes from two sources, with one code path:
//! the initial-table scan (respawn: the block cache / filesystem control-
//! channel end is in the initial table) and `NEW_CHANNEL` events on the
//! console channel (boot: the kernel injects the client endpoint after the
//! driver started). The console channel is *not* an RD_* endpoint — it is
//! waited on only for control events.
//!
...
pub enum EndpointState {
    ...
    /// handshake, blockdevice.aeroidl contract).
    AwaitingHandshake,
    Active,
}
