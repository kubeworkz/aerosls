//! The set of NET_* endpoints the driver serves.
//!
//! Adoption is idempotent and comes from two sources: the initial-table
//! scan (respawn) and `NEW_CHANNEL` events on the console channel (boot).
//! The console channel is *not* a NET_* endpoint — it is waited on only
//! for control events.
//!
//! Per-endpoint persistent state is deliberately tiny: `{handle, state}`.
//! Window = 1 (kernel enforced) serializes each endpoint.

/// `limits.max_channels` (16) minus the console channel.
pub const MAX_NET_ENDPOINTS: usize = 15;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum EndpointState {
    /// First message on this endpoint must be `NET_INFO` (implicit handshake).
    AwaitingHandshake,
    Active,
}
...
pub struct EndpointSet {
    console: u32,
    eps: [Option<Endpoint>; MAX_NET_ENDPOINTS],
    n: usize,
}
...
impl EndpointSet {
    pub fn new(console: u32) -> EndpointSet {
        EndpointSet {
            console,
            eps: [None; MAX_NET_ENDPOINTS],
            n: 0,
        }
    }

    /// Adopt a channel as a NET_* endpoint. Idempotent. Returns `false` when
    /// the table is full — the caller should close the channel.
    pub fn adopt(&mut self, handle: u32) -> bool {
        if self.find(handle).is_some() {
            return true;
        }
        if self.n >= MAX_NET_ENDPOINTS {
            return false;
        }
    ...
    /// Handles to wait on: console first, then adopted endpoints.
    pub fn wait_list(&self) -> [u32; MAX_NET_ENDPOINTS + 1] {
        let mut list = [0u32; MAX_NET_ENDPOINTS + 1];
        list[0] = self.console;
