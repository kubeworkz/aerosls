//! The Device Manager flat binary (feature `target`).
//!
//! The real entry point is `_start` — the crt0 in the LIB (crt0.S via
//! `global_asm!` in entry.rs), placed at address 0 by dm.ld
...
//!
//! ```sh
//! cargo build -p aerosls-dm --features target \
//!     --target x86_64-unknown-none --release --bin dm
//! ```
//!
//! That produces `target/x86_64-unknown-none/release/dm` (ELF); flatten
...
#[cfg(target_os = "none")]
extern crate aerosls_dm;

#[cfg(not(target_os = "none"))]
fn main() {
    panic!("the dm sidecar binary is only buildable for x86_64-unknown-none");
}
