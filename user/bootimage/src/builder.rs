...
/// Network driver peer — the POSIX sidecar's network socket endpoint.
pub const NET_PEER: &str = "drv.network.0";
...
fn build_init_manifest(spec: &BootImageSpec, layout: &BootLayout, blob_offset: u32) -> Vec<u8> {
    let caps = [
        ...
        Some(ManifestCap {
            name: "net.image",
            rights: 0x1, // R — init reads the network binary out of it
            kind: CapKind::Mem {
                base: layout.net_image.phys,
...
/// Build the network driver's packed manifest — budget + console.
fn build_net_manifest(spec: &BootImageSpec, layout: &BootLayout, blob_offset: u32) -> Vec<u8> {
    let caps = [
        Some(ManifestCap {
            name: "budget",
            rights: 0x3, // R | W
            kind: CapKind::Mem {
                base: layout.net_heap.phys,
                size: spec.net_heap_bytes,
            },
        }),
        Some(ManifestCap {
            name: "console",
    ...
    let m = Manifest {
        ...
        image: Some(Image {
            offset: blob_offset,
            size: spec.net_bin.len() as u32,
            entry: spec.net_entry,
        }),
        budget: Some(Budget {
            mem_bytes: spec.net_heap_bytes,
            stack_bytes: 16 * 1024,
            heap_initial: 64 * 1024,
        }),
        cpu: Some(Cpu {
    ...
    pack_with_footer(&m, layout.net_image.phys)
}
...
pub fn build_boot_image(spec: &BootImageSpec) -> BootImage {
    ...
    let net_manifest_ph = build_net_manifest(spec, &layout, 0);
    ...
    let net_bin_off = entry_offsets[8].1 as u32;
    ...
    let net_manifest = build_net_manifest(spec, &layout, net_bin_off);
