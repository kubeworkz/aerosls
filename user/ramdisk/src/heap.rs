...
//! (implementation plan §3.2 step 2) initializes the heap, so it exists and
//! is unit-tested. It is intentionally *not* wired as the Rust global
//! allocator — nothing in the driver allocates.
...
mod tests {
    ...
    fn bump_alloc_aligns_and_exhausts() {
        ...
        // Every allocation honors its alignment, regardless of base address.
        assert_eq!(a % 8, 0);
        assert_eq!(b % 16, 0);
        assert_eq!(c % 32, 0);
        // Strictly increasing and in-bounds (no overlap, no overflow).
        assert!(a >= base && a < b && b < c);
        assert!(c + 1 <= base + (1024 - pad));
        // used() is cursor - base; remaining() is what is left.
        assert_eq!(h.used(), c + 1 - base);
        assert_eq!(h.remaining(), (base + (1024 - pad)) - (c + 1));

        // Exhaustion: a request bigger than the region fails...
        assert!(h.alloc(2048, 1).is_none());
        // ...and a request that still fits succeeds.
        assert!(h.alloc(16, 1).is_some());
        assert_eq!(h.used(), c + 1 - base + 16);
    }

    #[test]
    fn bump_reports_none_before_use() {
        let mut h = Bump::new();
        assert_eq!(h.alloc(16, 1), None); // uninitialized
        assert_eq!(h.used(), 0);
